...
Best,\nyour Auto-Rotate Developer\n""".format(ex_type.__name__, str(ex), stack_trace))
    ...
    def write_mesh(self, objects, info, outputfile, output_type="binarystl"):
        ...
        if output_type == "asciistl":
            ...
            for part, content in objects.items():
                ...
                mesh = self.rotate_ascii_stl(
                    info[part]["matrix"], mesh, filename)
                if len(objects.keys()) == 1:
                    outname = outputfile
                else:
                    outname = ".".join(outputfile.split(
                        ".")[:-1]) + "_{}.stl".format(part)
    ...
    def write_bin_facett(facett):
        ...
        tweaked += struct.pack("<fff",
                               facett[1][0], facett[1][1], facett[1][2])
        tweaked += struct.pack("<fff",
                               facett[2][0], facett[2][1], facett[2][2])
        tweaked += struct.pack("<fff",
                               facett[3][0], facett[3][1], facett[3][2])
